...
# 2
# from calendar import isleap # test
...
day_new = sum((_months[i+1] for i in range(month)), start=day+2) - _months[month]
# print(day_new) # test

_days = 366 if is_leap(year) else 365

year_z = year + 1 if day_new > _days else year

month_z = month
# print(f'_months[month] = {_months[month]}') # test
# print(f'day + 2 = {day + 2}') # test
month_z = month_z + 1 if _months[month] < day + 2 else month_z
month_z = 1 if day_new > _days else month_z
