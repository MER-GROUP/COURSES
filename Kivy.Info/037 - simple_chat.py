...
С файлом server.py всё. Теперь напишем файл client.py. В этом файле мы будем использовать Kivy. 
...
Далее в этом же классе ClientApp пишем метод on_stop. Этот метод вызывается когда 
мы закрываем приложение. Нужно чтобы когда клиент закрыл приложение то сервер 
получил сообщение что клиент покинул чат и передал это сообщение всем подключенным клиентам

Далее в этом же классе ClientApp пишем метод подключения к серверу connect_to_server
      
C классом ClientApp все. Далее пишем код чтобы при заупске файла 
вызывался метод run у класса ClientApp

Если вы все написали правильно то при запуске файла client.py должен открыться 
такое окно с клиентом
...
Далее откройте вторую консоль где запустим окно клиента. Введите команду python 
client.py. Введем никнейм(имя пользователя), ip сервера и порт сервера который 
написан в консоли у сервера. Далее нажмите на кнопку подключиться к серверу

Далее запустим еще одну консоль где запустим еще одного клиента. Введите так же в 
консоли команду python client.py и подключитесь к серверу как мы подключались с первым клиентом
