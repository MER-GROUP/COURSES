...
Примечание 1. Для решения задачи используйте анонимную функцию 
и необязательный аргумент key  функции sorted().
...
print(
    *sorted(
        mixed_list,
        key=lambda x: ((type(x) is str, x), (type(x) is int, x))
    )
)

print('----------------------------------------------------')
print(*sorted(mixed_list, key=lambda x: (x, str(x))[type(x) == int]))