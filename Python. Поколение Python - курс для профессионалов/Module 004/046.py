...
Реализуйте функцию extract_this(), которая принимает один 
или более аргументов в следующем порядке:

zip_name — название zip архива, например, data.zip
*args — переменное количество позиционных аргументов, 
каждый из которых является названием некоторого файла
...
должен извлечь из архива workbook.zip файлы earth.jpg и exam.txt 
...
содержащую только необходимую функцию extract_this(), 
но не код, вызывающий ее.
'''
from zipfile import ZipFile

def extract_this(zip_name: str, *args):
    with ZipFile(file=zip_name, mode='r') as zip_opener:
        if not len(args):
            zip_opener.extractall()
        else:
            # [zip_opener.extract(file) for file in args]
            for file in args:
                zip_opener.extract(file)