...
работающих в организации. В последующих n строках вводятся данные 
о каждом сотруднике: имя, фамилия и дата рождения, разделённые пробелом. 
...
программа должна вывести их все в порядке возрастания, 
каждую на отдельной строке, в том же формате.

Примечание. Тестовые данные доступны по ссылке.
https://stepik.org/media/attachments/lesson/571244/tests_2506826.zip
...
lst, itog = [input().rsplit(' ', 1) for i in range(int(input()))], {}
for i in lst:
    itog.setdefault(datetime.strptime(i[1], '%d.%m.%Y').date(), []).append(*i[:-1])

max_staff_in_date = len(itog[max(itog, key=lambda x: len(itog[x]))])

for k, v in sorted(itog.items()):
    if len(v) == max_staff_in_date:
