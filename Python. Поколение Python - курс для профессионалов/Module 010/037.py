...
пятым — год смерти. Если человек жив, год смерти считается равным 0. 
...
live_men = (i for i in persons if not i.death)
only_men = (i for i in live_men if 'male' == i.sex)
swe_men = (i for i in only_men if 'Swedish' == i.nationality)
yong_men = max(swe_men, key=lambda x: x.birth)
print(yong_men.name)